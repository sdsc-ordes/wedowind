...
def test_build_distributions_single_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = mod._build_distributions(
        [
            {
                "downloadURL": "https://example.org/file",
                "formatExtension": "txt",
                "compression": "none",
            }
        ]
...
def test_build_distributions_multi_adds_part(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = mod._build_distributions(
        [
            {"downloadURL": "u1", "formatExtension": "txt"},
            {"downloadURL": "u2", "formatExtension": "zip"},
        ]
    )
...
def test_build_distributions_rejects_missing_url() -> None:
    with pytest.raises((BadArgumentException, KeyError)):
        mod._build_distributions([{}])  # type: ignore[list-item]
